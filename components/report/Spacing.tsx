...
export function Spacing({ 
  ...
  const getSpacingClasses = (size: string, direction: string) => {
    const spacingMap = {
      ...
      "2xl": { vertical: "space-y-12", horizontal: "space-x-12", all: "gap-12" }
    };

    return spacingMap[size as keyof typeof spacingMap][direction as keyof typeof spacingMap.md];
  };
...
export function HorizontalStack({ 
  ...
  return (
    <div className={`flex items-center ${size === "xs" ? "space-x-1" : size === "sm" ? "space-x-2" : size === "md" ? "space-x-4" : size === "lg" ? "space-x-6" : size === "xl" ? "space-x-8" : "space-x-12"} ${className}`}>
      {children}
    </div>
  );
}
...
export function Card({ 
  ...
  return (
    <div className={`bg-white rounded-lg shadow-sm border border-neutral-200 ${getPaddingClasses(padding)} ${className}`}>
      {children}
    </div>
  );
}
...
export function generateTextReport(summary: PersonalizedSummary, reportId: string): string {
  const lines: string[] = [];
  ...
  lines.push('='.repeat(60));
  lines.push('');
  lines.push(`Generated on: ${summary.generatedAt.toLocaleDateString('en-HK', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
    hour: '2-digit',
...
export function generateTextFileName(summary: PersonalizedSummary, reportId: string): string {
  const date = summary.generatedAt.toISOString().split('T')[0];
  return `policy-summary-${reportId}-${date}.txt`;
}

export function downloadTextReport(summary: PersonalizedSummary, reportId: string): void {
  try {

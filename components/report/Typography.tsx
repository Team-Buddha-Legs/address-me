...
export function Typography({
  ...
  const getVariantClasses = (variant: string) => {
    switch (variant) {
      case "h1":
        return "text-2xl sm:text-3xl font-bold leading-tight";
      case "h2":
        return "text-xl sm:text-2xl font-semibold leading-tight";
      case "h3":
        return "text-lg sm:text-xl font-semibold leading-tight";
      case "h4":
  ...
  return (
    <Component
      className={`${getVariantClasses(variant)} ${getColorClasses(color)} ${className}`}
    >
      {children}
    </Component>
  );

...
import { handleServerAction, handleClientAction, AppError, ErrorType } from '@/lib/error-handling';
import ErrorBoundaryProvider, { useErrorHandler, useAsyncErrorHandler } from '@/components/error/ErrorBoundaryProvider';
...
function TestComponent() {
  ...
  const handleServerError = async () => {
    ...
    if (!result.success) {
      reportError(new AppError(result.error.message, result.error.type, result.error.statusCode));
    }
  };
...
function RenderErrorComponent({ shouldThrow = false }: { shouldThrow?: boolean }) {
  if (shouldThrow) {
...
describe('Error Handling Integration', () => {
  ...
  describe('Server Action Error Handling', () => {
    it('should handle server action errors correctly', async () => {
      const mockAction = vi.fn().mockRejectedValue(new AppError('Test server error', ErrorType.AI_SERVICE));
  ...
  describe('Client Action Error Handling', () => {
    it('should handle client action errors correctly', async () => {
      const mockAction = vi.fn().mockRejectedValue(new AppError('Test client error', ErrorType.NETWORK));
      
      await expect(handleClientAction(mockAction, 'client-context')).rejects.toThrow(AppError);
  ...
  describe('Error Recovery and Logging', () => {
    it('should log different error types with appropriate context', async () => {
      ...
      await handleServerAction(async () => { throw networkError; }, 'network-test');
      await handleServerAction(async () => { throw aiError; }, 'ai-test');
      await handleServerAction(async () => { throw validationError; }, 'validation-test');
    ...
    it('should handle error normalization correctly', async () => {
      ...
      await handleServerAction(async () => { throw stringError; }, 'string-test');
      await handleServerAction(async () => { throw objectError; }, 'object-test');
      await handleServerAction(async () => { throw nullError; }, 'null-test');

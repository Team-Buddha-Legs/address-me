...
describe('Error Handling', () => {
  ...
  describe('Custom Error Classes', () => {
    it('should create AppError with correct properties', () => {
      const error = new AppError('Test error', ErrorType.VALIDATION, 400, true, { field: 'test' });
  ...
  describe('handleClientAction', () => {
    ...
    it('should throw normalized error for failed action', async () => {
      ...
      await expect(handleClientAction(action, 'client context')).rejects.toThrow(AppError);
    });
  });
  ...
  describe('Error Message Utilities', () => {
    it('should return user-friendly messages for different error types', () => {
      expect(getErrorMessage(new NetworkError('Network failed')))
        .toBe('Connection error. Please check your internet connection and try again.');
      
      expect(getErrorMessage(new AIServiceError('AI failed')))
        .toBe('AI service is temporarily unavailable. Please try again in a few minutes.');
      
      expect(getErrorMessage(new RateLimitError('Rate limit')))
        .toBe('Too many requests. Please wait a moment before trying again.');
      
      expect(getErrorMessage(new ValidationError('Invalid email')))
        .toBe('Invalid email');
      
      expect(getErrorMessage(new AppError('Unknown', ErrorType.UNKNOWN)))
        .toBe('An unexpected error occurred. Please try again.');
    });

    it('should return appropriate titles for different error types', () => {
      expect(getErrorTitle(new NetworkError('Network failed'))).toBe('Connection Error');
      expect(getErrorTitle(new AIServiceError('AI failed'))).toBe('Service Unavailable');
      expect(getErrorTitle(new RateLimitError('Rate limit'))).toBe('Too Many Requests');
      expect(getErrorTitle(new ValidationError('Invalid'))).toBe('Invalid Input');
      expect(getErrorTitle(new AppError('Unknown', ErrorType.UNKNOWN))).toBe('Error');
    });
  });
